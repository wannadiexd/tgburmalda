...
async def game_selected_text(msg: Message, state: FSMContext):
    """Выбор игры по названию"""
    game_map = {
        "🏀 Баскетбол": "🏀",
        "🎲 Кости": "🎲",
        "⚽ Футбол": "⚽",
...
@router.message(StateFilter(None), lambda msg: msg.text and msg.text.isdigit())
async def bet_type_selected(msg: Message, state: FSMContext):
    """Выбор типа ставки по номеру"""
    logger.info(f"bet_type_selected вызван")
    ...
    choice = int(msg.text)
    bet_mapping = {
        '🏀': {1: 'гол', 2: 'застрял', 3: 'мимо'},
        '🎲': {1: 'четное', 2: 'нечетное', 3: 'больше_3', 4: 'меньше_4'},
        '⚽': {1: 'гол', 2: 'мимо'},
        '🎯': {1: 'центр', 2: 'красное', 3: 'белое', 4: 'мимо'},
        '🎳': {1: 'страйк', 2: 'мимо'}
    }
    
    bet_type = bet_mapping.get(game, {}).get(choice)
    
    if not bet_type:
        return await msg.answer("❌ Неверный выбор. Попробуй снова.")
    ...
    await msg.answer(
        ...
        f"Введи сумму ставки (1-2500):"
    )


@router.message(StateFilter(None), lambda msg: msg.text and msg.text.replace('-', '').isdigit())
async def amount_entered(msg: Message, state: FSMContext, bot: Bot):
    """Ввод суммы ставки"""
    # Проверяем что это не команда addbalance
    ...
    try:
        ...
        if ud['balance'] >= amount:
            ...
            await bot.send_invoice(
                ...
                description=f"Ставка {amount} ⭐ на {game} ({bet_type})",
                payload=f"{msg.from_user.id}:{game}:{bet_type}:{amount}",
                currency="XTR",
                prices=[LabeledPrice(label=f"{game} {bet_type}", amount=amount)]
            )
            await state.clear()
